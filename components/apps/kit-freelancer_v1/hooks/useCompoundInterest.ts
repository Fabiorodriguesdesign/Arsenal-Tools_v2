...
import { calculateCompoundInterest, InvestmentType, PeriodUnit } from '../utils/financial';
...
export const useCompoundInterest = () => {
    ...
    const setPeriodUnit = (val: PeriodUnit) => setFormData(prev => ({ ...prev, periodUnit: val }));
    const setInvestmentType = (val: InvestmentType) => setFormData(prev => ({ ...prev, investmentType: val }));
